...
```bash
python run\\\_agents\\\_v2.py run /ruta/a/mi-proyecto "Implementar login con JWT"
```
...
(`sdd/`), que reemplaza la dependencia al MCP externo previo del SDD (framework ajeno):
...
* **`RUN\\\_AGENTS\\\_V2\\\_GUIDE.md`** — referencia completa del script: flags, stack, timeout, env vars, troubleshooting, historial técnico.
...
agentes (default `\\\~/.config/opencode/agents/`, o la env `OPENCODE\\\_AGENTS\\\_DIR`).
...
sección en `.opencode-context.md` y el marker `<!-- AGENT\\\_DONE: <rol> -->`).
...
* **Deja su marker** `<!-- AGENT\\\_DONE: <rol> -->` en la ÚLTIMA línea.
...
## Regla de markers
1. \\\*\\\*JAMÁS escribas el marker de otro agente\\\*\\\* — ni `<!-- AGENT\\\_DONE: explorer -->`,
   ni `<!-- AGENT\\\_DONE: tester -->`, ni ningún otro.
2. \\\*\\\*Solo escribes TU marker:\\\*\\\* `<!-- AGENT\\\_DONE: coder -->`. Nada más.
```
...
Si existe `<rol>-<stack>.md` (ej. `coder-maven.md`, `tester-html\\\_playwright.md`),
